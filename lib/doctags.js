...
const { SymbolFlags, ModifierFlags, ContainerFlags } = require( './types' );
...
class Symbol
{
    ...
    constructor( name, node )
    {
        this.name = name;
        this.node = node;
        this.scope = Scope.get_scope( node ).add( name, this );
        this._flags = SymbolFlags.None;
        this._modifiers = ModifierFlags.None;
    }
    ...
    set flags( v )
    {
        this._flags = SymbolFlags.create( +this._flags | v );
    }
    ...
    set modifiers( v )
    {
        this._modifiers = ModifierFlags.create( +this._modifiers | v );
    }
...
function method_name( node )
{
    ...
    if ( id.type === Syntax.MemberExpression )
    {
        if ( !id.computed && id.object.type === Syntax.Identifier && id.object.name === 'Symbol' )
            return '@@' + id.property.name;
        else if ( id.computed )
            return '<computed[' + id.property.name + ']>';
    }
...
function create_symbol( node, cb )
{
    ...
    switch ( node.type )
    {
        ...
        case Syntax.MethodDefinition:
            ...
            if ( node.generator ) sym.modifiers |= ModifierFlags.GeneratorFunction;
            if ( node.async ) sym.modifiers |= ModifierFlags.Async;

            if ( node.computed || node.key.computed )
            {
                sym.modifiers |= ModifierFlags.HasComputedFlags;
                sym.as( SymbolFlags.Property | SymbolFlags.Method );
            }
            break;
...
function function_expression( node )
{
    ...
    else if ( p.type === Syntax.MethodDefinition )
    {
        ...
        if ( p.kind === 'get' )
            sym.as( SymbolFlags.GetAccessor );
...
module.exports = build_definition;
