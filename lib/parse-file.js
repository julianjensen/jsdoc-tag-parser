...
function prep( withComments, file )
{
    const
        ...
        allNodesParsed = [],
        byIndex        = [],
        scopes         = new Scopes( withComments );
    ...
    traverse( withComments, {
        enter( node, parent )
        {
            Object.defineProperty( node, 'parent', { enumerable: false, value: parent } );

            node.index = byIndex.length;
            node.containerFlags = get_container_flags( node );

            let scope = scopes.get_scope( node ),
                p     = node.parent;

            while ( !scope && p )
            {
                scope = scopes.get_scope( p );
                p = p.parent;
            }

            Object.defineProperty( node, 'scope', { enumerable: false, value: scope } );
    ...
    const r = {
        fileName:    file,
        types:       [ ...types ],
        allDocNodes: allNodesParsed,
        inline:      [ ...table.values() ]
    };

    Object.defineProperty( r, 'scopes', { enumerable: false, value: scopes } );
...
function get_node_name( node )
{
    switch ( node.type )
    {
        ...
        case Syntax.FunctionExpression:
            ...
            return `<anonymous[${node.type}@${node.index}]>`;
        ...
        case Syntax.NewExpression:
            return `<transient[${node.type}@${node.index}]>`;
        ...
        case Syntax.ClassExpression:
            if ( node.id ) return get_node_name( node.id );
                ...
                return get_node_name( node.parent );
            return `<anonymous[ClassExpression@${node.index}]>`;
